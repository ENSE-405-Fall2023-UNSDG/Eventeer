...
export default function BadgeFromPage(){
    ...
    useEffect(() => {
        ...
        axios.get('/badgepage/edit/' + id).then(response => {
            const {data} = response;
            setTitle(data.title);
    ...
    async function saveBadge(ev){
        ev.preventDefault();

        if (id){
            await axios.put('/badges', {id, title, addedPhoto, 
                description, date});

                setRedirectToEventList(true);

        }else{
            await axios.post('/badges', {title, addedPhoto, 
                description, date});
    ...
    if (redirectToEventList) {
        return <Navigate to={'/profilepage/badgepage'} />
    }
    ...
    return (
        ...
        <div>
            ...
            <form className="new-event-from" onSubmit={saveBadge}>
                <h2>Title</h2>
                <input type="text" value={title} onChange={ev => setTitle(ev.target.value)} placeholder="Badge name"/>
                <h2>Photos</h2>
